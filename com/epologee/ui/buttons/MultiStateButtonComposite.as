package com.epologee.ui.buttons {
	import com.epologee.util.TimelineUtils;		import flash.display.FrameLabel;	import flash.display.MovieClip;	import flash.display.Sprite;	import flash.events.Event;	import flash.events.MouseEvent;	import flash.utils.getQualifiedClassName;			/**
	 ...
	 * 
	 */
	public class MultiStateButtonComposite extends Sprite {
		 ...
		 */
		public function MultiStateButtonComposite(inTimeline:MovieClip, inStartInstantly : Boolean = true) {
			_timeline = inTimeline;
			
			// don't handle mouse events on children
			_timeline.mouseChildren = false;
			// set and hide hit area
			if (_timeline.tHitArea != null) {
				_timeline.hitArea = _timeline.tHitArea;
				_timeline.tHitArea.visible = false;
			}
			
			if (_timeline.tAlternativeState) {
				_timeline.tAlternativeState.visible = false;
			}			
			...
			if (inStartInstantly && _timeline.name.substr(-6) != "Paused") {
				if (_timeline.stage) {
					handleAddedToStage();
				} else {
			...
			TimelineUtils.reparentSprite(this, _timeline);
		}
		...
		public function showAlternativeState(inShow : Boolean) : void {
			if (inShow && _timeline.tAlternativeState) {
				_timeline.tAlternativeState.visible = true;
			} else if (_timeline.tAlternativeState) {
				_timeline.tAlternativeState.visible = false;
			}
			
			if (_timeline.tAlternativeState && _timeline.tRegularState) {
				_timeline.tRegularState.visible = !_timeline.tAlternativeState.visible;
			}
		}
		...
		public function disable(inDisabledMouseState : int = -1) : void {
			...
			_timeline.hitArea = null;
			
			if (inDisabledMouseState > MOUSE_INACTIVE) {
				stackMouse(inDisabledMouseState);
			}
		}
